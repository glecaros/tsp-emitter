...
function getDecoratorArg(element: Decorated, decoratorName: string, predicate: (args: DecoratorArgument[]) => boolean): DecoratorArgument[] | undefined {
...
export function getEncodedName(element: Decorated, mimeType: string): Optional<string> {
    const encodedName = getDecoratorArg(element, "@encodedName", args => args.length === 2 && args[0].jsValue === mimeType);
    return encodedName?.at(0)?.jsValue as Optional<string>;
}
